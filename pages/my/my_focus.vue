	...
	<view>
		<view v-for="(focus, index) in my_focus" :key="index" class="post-item">
			...
			<view class="user-info">
				<!-- 头像 -->
				<image class="user-avatar" :src="my_focus[index].avatar_url" mode="aspectFill" @click="goToUserHome_focus(index)"></image>
				<!-- 用户名和关注按钮 -->
...
<script>
	export default {
		...
		methods: {
			goToUserHome_focus(index) {
				const focusUuid = this.my_focus[index].uuid;
				uni.navigateTo({
					url: `/pages/user-home/user-home?uuid=${focusUuid}`,
				});
			},
			follow_click(index) {
				console.log("a");
