	...
	<view>
	    <view v-for="(focus, index) in my_focus" :key="index" class="post-item">
	      ...
	      <view class="user-info">
	        <!-- 头像 -->
	        <image class="user-avatar" :src="my_focus[index].avatar_url" mode="aspectFill"></image>
	        <!-- 用户名和关注按钮 -->
	        <view class="user-details">
	          <view class="user-details-top">
	            <text class="user-name">{{ my_focus[index].username }}</text>
				<button :class="{'follow-button': if_focus[index], 'unfollowed-button': !if_focus[index]}" @click="follow_click(index)">{{ if_focus[index] ? '已关注' : '关 注'}}</button>
	          </view>
...
.user-info {
	...
	.user-avatar {
	  ...
	  height: 50px;
	  border-radius: 50%;
	  margin-right: 15px; /* 增加头像和用户名之间的间距 */
	}
	...
	.user-details-top {
	  display: flex;
	  align-items: center;
	  justify-content: space-between; /* 将用户名和按钮放在同一行 */
	  margin-bottom: 6px; /* 调整按钮与介绍之间的间距 */
	}
</style>
